...
int getProg_callback ( void *data, int argc, char **argv, char **azColName ) {
    ...
    DB_FOREACH_COLUMN {
        if ( DB_COLUMN_IS ( "id" ) ) {
	        ...
	        item->goal = DB_CVF;
            c++;
        } else if ( DB_COLUMN_IS ( "delta" ) ) {
            item->delta = DB_CVF;
            c++;
		} else if ( DB_COLUMN_IS ( "duty_cycle" ) ) {
            item->duty_cycle = DB_CVF;
            c++;
            ...
            c++;
        } else if ( DB_COLUMN_IS ( "interval_read_sec" ) ) {
            item->interval_read.tv_sec = DB_CVI;
            item->interval_read.tv_nsec = 0;
            c++;
        } else if ( DB_COLUMN_IS ( "interval_set_sec" ) ) {
            item->interval_set.tv_sec = DB_CVI;
            item->interval_set.tv_nsec = 0;
            c++;
        } else {
...
int getChannel_callback ( void *data, int argc, char **argv, char **azColName ) {
    ...
    DB_FOREACH_COLUMN {
        if ( DB_COLUMN_IS ( "id" ) ) {
            ...
            c++;
        } else if ( DB_COLUMN_IS ( "temp_sensor_remote_channel_id" ) ) {
            item->sensor_temp.remote_channel.id=DB_CVI;
            if ( !config_getRChannel ( &item->sensor_temp.remote_channel, item->sensor_temp.remote_channel.id, db, NULL ) ) {
                return EXIT_FAILURE;
            }
            c++;
		} else if ( DB_COLUMN_IS ( "hum_sensor_remote_channel_id" ) ) {
            item->sensor_hum.remote_channel.id=DB_CVI;
            if ( !config_getRChannel ( &item->sensor_hum.remote_channel, item->sensor_hum.remote_channel.id, db, NULL ) ) {
                return EXIT_FAILURE;
            }
            c++;
		} else if ( DB_COLUMN_IS ( "fly_sensor_remote_channel_id" ) ) {
            item->sensor_fly.remote_channel.id=DB_CVI;
            if ( !config_getRChannel ( &item->sensor_fly.remote_channel, item->sensor_fly.remote_channel.id, db, NULL ) ) {
                return EXIT_FAILURE;
            }
            c++;
		} else if ( DB_COLUMN_IS ( "reg_remote_channel_id" ) ) {
            item->reg.remote_channel.id=DB_CVI;
            item->reg.retry_num = RETRY_COUNT;
            if ( !config_getRChannel ( &item->reg.remote_channel, item->reg.remote_channel.id, db, NULL ) ) {
                return EXIT_FAILURE;
            }
            c++;
		} else if ( DB_COLUMN_IS ( "flyte_remote_channel_id" ) ) {
            item->flyte.remote_channel.id=DB_CVI;
            item->flyte.retry_num = RETRY_COUNT;
            if ( !config_getRChannel ( &item->flyte.remote_channel, item->flyte.remote_channel.id, db, NULL ) ) {
                return EXIT_FAILURE;
            }
            c++;
